...
public class ChessProject extends JFrame implements MouseListener, MouseMotionListener {
    ...
    Boolean agentwins;
    Stack temporary;
    static int difficulty; //This is my variable to track which ai the user will wish to face
...
private Stack getWhitePawnSquares(int x, int y, String piece){
  ...
  Square startSquarePos = new Square(x, y, piece);
  Move validM;
  ...
  if(!(tempY > 7 || tempY < 0)){
    ...
    validM = new Move(startSquarePos, temp);
    if(!piecePresent(((temp.getXC()*75)+20), ((temp.getYC()*75)+ 20))){
      moves.push(validM);
    }
    else if(checkWhiteOponent(((temp.getXC()*75)+20),((temp.getYC()*75)+20))){
      moves.push(validM);
    }
  }
  return moves;
